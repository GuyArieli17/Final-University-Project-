...
class ClusterMannager:
    ...
    def push_car_to_cluster_info(self,distance: float,lane_idx: int,road_idx: int,value: float):
        ...
        cluster_length = self.division_length[category_index]
        cluster_index = int((abs((distance - category_length))//cluster_length))
        ...
        for road_id in self.traffic_road_id:
            for category in self.mapper[road_id]:
                for clusster in category:
                    if clusster[1]:
                        output[0].append(clusster[0] / clusster[1])
                        output[1].append(clusster[1])
                    else: 
                        output[0].append(0)
                        output[1].append(0)
